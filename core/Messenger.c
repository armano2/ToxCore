...
uint32_t m_sendmessage(Messenger *m, int friendnumber, uint8_t *message, uint32_t length)
{
    ...
    return m_sendmessage_withid(m, friendnumber, msgid, message, length);
}
...
void m_callback_friendstatus(Messenger *m, void (*function)(Messenger *m, int, uint8_t))
{
...
static void set_friend_status(Messenger *m, int friendnumber, uint8_t status)
{
    if (m->friendlist[friendnumber].status != status && m->friend_statuschange_isset)
        m->friend_statuschange(m, friendnumber, status);
